...
class BeamSynthesiser:
    def __init__(self, X: Dict[str, np.ndarray], y: np.ndarray, beam_width: int = 400):
    ...
    def mae(self, col: np.ndarray) -> float:
        return float(np.mean(np.abs(col - self.y)))
...
def main():
    X, y = load_public()
    synth = BeamSynthesiser(X, y, beam_width=400)
    best_stmt = synth.search(max_iters=6)
